...
class BertRelationalSelfAttention(BertSelfAttention):
    def __init__(self, *args, num_relation_kinds: int, use_same_relation_kv_emb: bool = True, **kwargs):
        ...
        self.relation_k_emb = nn.Embedding(num_relation_kinds + 1, self.attention_head_size // self.num_attention_heads, padding_idx=0)
        if use_same_relation_kv_emb:
            self.relation_v_emb = self.relation_k_emb
        else:
            self.relation_v_emb = nn.Embedding(num_relation_kinds + 1, self.attention_head_size // self.num_attention_heads, padding_idx=0)
